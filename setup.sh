...
setup_cohort_view() {
    print_header "🏥 COHORT VIEW SETUP"
    if [[ -f "cohort_disease.sql" ]]; then
        print_step "Creating materialized view from cohort_disease.sql"
        if psql "$DATABASE_URL" -f cohort_disease.sql; then
            print_success "cohort_disease view created"
        else
            print_error "Failed to create cohort_disease view"
        fi
    else
        print_warning "cohort_disease.sql file not found"
    fi
}
...
full_setup() {
    ...
    if validate_pipeline; then
        ...
        python analyze_statistics.py || print_warning "Statistical analysis failed"

        print_step "Generating visualizations..."
        python visualizations.py || print_warning "Visualization generation failed"
